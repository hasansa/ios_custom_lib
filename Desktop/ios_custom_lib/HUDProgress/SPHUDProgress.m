...
#import "SPHUDProgress.h"

#import "UIImage+SPHUDImageEffects.h"

typedef NS_ENUM(NSUInteger, SPHUDProgressStyle) {
	SPHUDProgressStyleHidden,
	SPHUDProgressStyleProgress,
	SPHUDProgressStyleSuccess,
	SPHUDProgressStyleError
};

NSString * const SPHUDProgressViewParameterFullScreen = @"SPHUDProgressViewParameterFullScreen";
NSString * const SPHUDProgressViewParameterBackgroundType = @"SPHUDProgressViewParameterBackgroundType";
NSString * const SPHUDProgressViewParameterStatus = @"SPHUDProgressViewParameterStatus";
NSString * const SPHUDProgressViewParameterSuperview = @"SPHUDProgressViewParameterSuperview";

static CGFloat const SPHUDFadeAnimationDuration = 0.3f;
static CGFloat const SPHUDLayoutAnimationDuration = 0.3f;
static CGFloat const SPHUDTextUpdateAnimationDuration = 0.5f;
static CGFloat const SPHUDCheckmarkAnimationDuration = 0.5f;
static CGFloat const SPHUDInfiniteLoopAnimationDuration = 1.0f;
static CGFloat const SPHUDProgressAnimationDuration = 0.25f;
static CGFloat const SPHUDProgressIndeterminate = CGFLOAT_MAX;
static CGFloat const SPHUDCircleProgressViewToStatusLabelVerticalSpaceConstraintConstant = 20.0f;
static CGFloat const SPHUDContentViewFullScreenModeLeadingAndTrailingSpaceConstraintConstant = 0.0f;
static CGFloat const SPHUDContentViewNotFullScreenModeLeadingAndTrailingSpaceConstraintConstant = 55.0f;
static CGFloat const SPHUDContentViewWithStatusInset = 10.0f;
static CGFloat const SPHUDContentViewWithoutStatusInset = 20.0f;
static CGFloat const SPHUDContentViewCornerRadius = 8.0f;
static CGFloat const SPHUDContentViewWithoutStatusCornerRadius = 15.0f;
static CGFloat const SPHUDAlertViewWidth = 270.0f;
static CGFloat const SPHUDMotionEffectRelativeValue = 10.0f;

@interface SPHUDProgress ()

@property (nonatomic) CGFloat progress;
@property (nonatomic) SPHUDProgressBackgroundType backgroundType;
@property (nonatomic) NSString *status;
@property (nonatomic, getter = isFullScreen) BOOL fullScreen;
@property (nonatomic) SPHUDProgressStyle style;
...
@implementation SPHUDProgress

#pragma mark - Shared

+ (SPHUDProgress *)sharedView
{
	static SPHUDProgress *sharedView = nil;
	static dispatch_once_t onceToken;
	
	dispatch_once(&onceToken, ^{
		UINib *nib = [UINib nibWithNibName:@"SPHUDProgressView"
									bundle:nil];
...
{
	[self showHUDWithProgress:SPHUDProgressIndeterminate
						style:SPHUDProgressStyleProgress
...
{
	[self showHUDWithProgress:SPHUDProgressIndeterminate
						style:SPHUDProgressStyleSuccess
				   parameters:parameters];
}
...
+ (void)showHUDWithProgress:(CGFloat)progress
					  style:(SPHUDProgressStyle)style
				 parameters:(NSDictionary *)parameters
{
	[[self sharedView] showProgress:progress
							 status:parameters[SPHUDProgressViewParameterStatus]
							  style:style
					 backgroundType:(SPHUDProgressBackgroundType)[parameters[SPHUDProgressViewParameterBackgroundType] unsignedIntegerValue]
						 fullScreen:[parameters[SPHUDProgressViewParameterFullScreen] boolValue]
							   view:parameters[SPHUDProgressViewParameterSuperview]];
}

- (void)showProgress:(CGFloat)progress
			  status:(NSString *)status
			   style:(SPHUDProgressStyle)style
	  backgroundType:(SPHUDProgressBackgroundType)backgroundType
		  fullScreen:(BOOL)fullScreen
				view:(UIView *)superview
{
    dispatch_async(dispatch_get_main_queue(), ^{
	__block SPHUDProgress *__blockSelf = self;
	...
	if (![self isWaitingToChangeHUD] && self.style != SPHUDProgressStyleHidden
		&& !(self.style == SPHUDProgressStyleProgress && self.progress == SPHUDProgressIndeterminate && progress != SPHUDProgressIndeterminate)) {
	...
	if ([self.class isVisible]) {
		[UIView animateWithDuration:SPHUDLayoutAnimationDuration
						 animations:^{
							 [__blockSelf setupUI];
						 }];
	...
	if (self.style != SPHUDProgressStyleProgress) {
		NSTimeInterval delay;
		switch (self.style) {
			case SPHUDProgressStyleProgress:
				// should never happen
				return;
			case SPHUDProgressStyleSuccess:
				delay = SPHUDMinimumSuccessDisplayTime;
				break;
			case SPHUDProgressStyleError:
				delay = SPHUDMinimumErrorDisplayTime;
				break;
			case SPHUDProgressStyleHidden:
				// should never happen
				return;
		}
...
{
	SPHUDProgress *progressView = [self sharedView];

	[UIView animateWithDuration:SPHUDFadeAnimationDuration
						  delay:0
					 ...
					 animations:^{
						 ...
						 if(progressView.alpha == 0 || progressView.contentView.alpha == 0) {
							 if ([progressView isDismissing]) {
								 ...
								 progressView.style = SPHUDProgressStyleHidden;
...
{
	CGFloat statusInset = (self.status.length > 0) ? SPHUDContentViewWithStatusInset : SPHUDContentViewWithoutStatusInset;
	CGFloat contentMargin = ([self isFullScreen]) ? SPHUDContentViewFullScreenModeLeadingAndTrailingSpaceConstraintConstant : SPHUDContentViewNotFullScreenModeLeadingAndTrailingSpaceConstraintConstant;
	
	if ([self isFullScreen]) {
		contentMargin = SPHUDContentViewFullScreenModeLeadingAndTrailingSpaceConstraintConstant;
	} else {
		CGFloat contentWidth = CGRectGetWidth([UIScreen mainScreen].bounds) - (2 * SPHUDContentViewNotFullScreenModeLeadingAndTrailingSpaceConstraintConstant);
		
		if (contentWidth > SPHUDAlertViewWidth) {
			contentMargin = (CGRectGetWidth([UIScreen mainScreen].bounds) - SPHUDAlertViewWidth) / 2.0f;
		}
	}
...
- (void)setupStatus:(NSString *)status
{
	...
	animation.duration = SPHUDTextUpdateAnimationDuration;
	...
	self.circleProgressViewToStatusLabelVerticalSpaceConstraint.constant = (showStatus) ? SPHUDCircleProgressViewToStatusLabelVerticalSpaceConstraintConstant : 0.0f;
...
- (void)setupBackground
{
	...
	switch (self.backgroundType) {
		case SPHUDProgressBackgroundTypeSolid:
			backgroundImage = [[UIImage alloc] init];
			backgroundColor = self.backgroundFillColor;
			break;
		case SPHUDProgressBackgroundTypeBlurred:
	...
	else
	{
		if (self.status.length == 0) {
			self.circleProgressViewTopToSuperViewConstraint.constant = SPHUDContentViewWithoutStatusInset;
			self.statusLabelBottomToSuperViewConstraint.constant = SPHUDContentViewWithoutStatusInset;
		...
		self.contentView.layer.cornerRadius = (self.status) ? SPHUDContentViewCornerRadius : SPHUDContentViewWithoutStatusCornerRadius;
			  ...
			  animated:(BOOL)animated
{
	...
	if (animated) {
		...
		progressAnimation.duration = SPHUDProgressAnimationDuration;
...
- (void)animateCircleWithInfiniteLoop
{
	...
	rotationAnimation.toValue = @(M_PI * 2.0f * SPHUDInfiniteLoopAnimationDuration);
	rotationAnimation.duration = SPHUDInfiniteLoopAnimationDuration;
...
- (void)animateSuccess
{
	...
	checkmarkAnimation.duration = SPHUDCheckmarkAnimationDuration;
...
- (void)animateFullCircleWithColor:(UIColor *)color
{
	...
	if (self.superview) {
		...
		circleAnimation.duration = SPHUDCheckmarkAnimationDuration;
...
{
	NSDictionary *fixedParameters = @{SPHUDProgressViewParameterBackgroundType: @(SPHUDProgressBackgroundTypeBlurred),
									  SPHUDProgressViewParameterFullScreen: @(NO)};
	NSMutableDictionary *parameters = [NSMutableDictionary dictionaryWithDictionary:fixedParameters];
	
	if (status) {
		parameters[SPHUDProgressViewParameterStatus] = status;
	}
											  ...
											  keyPath:(NSString *)keypath
{
	...
	motionEffect.minimumRelativeValue = @(-SPHUDMotionEffectRelativeValue);
	motionEffect.maximumRelativeValue = @(SPHUDMotionEffectRelativeValue);
...
{
	return (self.progress == SPHUDProgressIndeterminate);
}
