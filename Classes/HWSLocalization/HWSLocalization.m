...
@implementation HWSLocalization {
    NSString * _language;
}
...
{
    language = [[HWSUserDefaults sharedDefaults] language];
...
+ (BOOL)isRTL
{
    return ([[[HWSUserDefaults sharedDefaults] language] isEqualToString:@"he"] || [[[HWSUserDefaults sharedDefaults] language] isEqualToString:@"ar"]);
}
...
{
    return [self stringForKey:key language:[[HWSUserDefaults sharedDefaults] language]];
}

+ (NSString *)stringForKey:(NSString *)key
{
    NSString* retString = [[HWSLocalization sharedInstance] stringForKey:key];
    return (retString != nil)? retString : key;
}

+ (NSString *)stringForKey:(NSString *)key withPlaceholders:(NSDictionary *)placeholders
{
    return [[HWSLocalization sharedInstance] stringForKey:key withPlaceholders:placeholders];
}
