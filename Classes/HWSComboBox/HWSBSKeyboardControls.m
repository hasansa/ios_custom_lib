...
@interface HWSBSKeyboardControls ()
...
@implementation HWSBSKeyboardControls
...
#endif // USE_SEGMENT
        [self setDoneButton:[[UIBarButtonItem alloc] initWithTitle:NSLocalizedStringFromTable(@"Done", @"HWSBSKeyboardControls", @"Done button title.")
...
#else
        [self setVisibleControls:HWSBSKeyboardControlDone];
#endif // USE_SEGMENT
...
- (void)setFields:(NSArray *)fields
{
    if (fields != _fields)
    {
        for (UIView *field in fields)
        {
            ...
            else if ([field isKindOfClass:[HWSCombobox class]])
            {
                [(HWSCombobox *)field setInputAccessoryView:self];
            }
        }
...
- (void)setVisibleControls:(HWSBSKeyboardControl)visibleControls
{
...
- (NSArray *)toolbarItems
{
    ...
    if (self.visibleControls & HWSBSKeyboardControlPreviousNext)
    {
        [items addObject:self.segmentedControlItem];
    }
    
    if (self.visibleControls & HWSBSKeyboardControlDone)
    {
